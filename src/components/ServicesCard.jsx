...
const ServicesCard = ({ service }) => {
  ...
  return (
    <div className="relative overflow-hidden max-w-lg m-2 sm:m-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl shadow-gray-100 dark:shadow-white/10" onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)} onMouseMove={handleMouseMove}>
      <div
      ...
      <div className="flex items-center gap-10 p-8 hover:p-7.5 hover:m-0.5 transition-all rounded-[10px] bg-white dark:bg-gray-900 z-10 relative">
        <div className="bg-gray-100 dark:bg-gray-700 rounded-full">
          <img
            className="max-w-24 bg-white dark:border-gray-900 rounded-full m-2"
            src={service.logo}
            alt=""
          />
        </div>
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-sm mt-2">{service.info}</p>
        </div>
      </div>
    </div>
  );
